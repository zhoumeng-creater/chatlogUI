...
计划性质：代码修复计划，不包含完整生产代码；后续实现必须以测试、浏览器证据、及时提交、及时推送和 PR 收口。
...
这一步应作为独立修复 PR 或少量紧密 PR 推进。不能等搜索闭环全部修完才一次性提交、推送和开 PR。每个可验证的小单元完成后都要中文短提交、推送，并创建或更新 draft PR。
...
- `verification-before-completion`：提交、推送、PR 和完成声明前必须有新鲜验证证据。
- `requesting-code-review` 和 GitHub 发布流程：实现 PR 前后进行本地 review，且及时提交、推送、开 draft PR。
...
建议提交：`修搜索请求防旧响应覆盖`。完成 focused tests 后立刻提交并推送；如果本阶段是第一段代码实现，立刻开 draft PR。
...
建议提交：`建立搜索命中定位合同`。该提交应在任务 1 PR 上追加并推送，或在冲突风险较高时拆成第二个 draft PR。
...
建议提交：`补聊天命中锚点加载`。提交前必须跑 focused L2/L4 tests；提交后推送。
...
建议提交：`补搜索结果高亮和返回路径`。提交前跑 chat component/helper tests；提交后推送。
...
建议提交：`完善搜索页面结果区状态`。提交前跑 search component tests 和浏览器 smoke；提交后推送。
...
建议提交：`补搜索闭环验收证据`。提交后推送；PR 描述必须列出自动化和浏览器证据。
...
这部分是 Step 06 实施硬要求，不是建议：

- 不允许把所有搜索修复攒到最后一次提交。
- 每个任务或每个可验证小单元完成后，先跑 focused tests，再中文短提交。
- 第一段可运行代码完成并通过 focused tests 后，立即推送分支并创建 draft PR。
- 后续任务继续以中文短提交推送到同一个 PR；如果任务文件范围独立且风险不同，可以拆成多个 draft PR。
- 每次提交前只 stage 本任务相关文件；如果工作树包含 unrelated changes，必须确认提交范围。
- 文档在 `docs/*` 下被 `.gitignore` 忽略，计划文档或后续文档更新需要 `git add -f`。
- PR 描述必须按问题 ID 写清：P1-04、P1-05 覆盖范围，未覆盖范围，验证命令，浏览器视口，隐私检查和剩余风险。

建议提交拆分：

| 顺序 | 内容 | 中文提交名示例 | PR 行为 |
| --- | --- | --- | --- |
| 1 | 搜索请求 snapshot、abort、stale guard | `修搜索请求防旧响应覆盖` | 通过 focused tests 后推送并开 draft PR |
| 2 | 搜索结果锚点导航合同 | `建立搜索命中定位合同` | 推送到 PR，更新 PR 描述 |
| 3 | chat history anchor 加载 | `补聊天命中锚点加载` | 推送到 PR，补 focused tests |
| 4 | MessageList 高亮和返回搜索 | `补搜索结果高亮和返回路径` | 推送到 PR，补 UI/E2E |
| 5 | 搜索页面结果区和证据 | `补搜索闭环验收证据` | 推送到 PR，准备 review |
