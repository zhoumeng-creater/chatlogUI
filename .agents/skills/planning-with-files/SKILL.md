...
hooks:
  ...
  Stop:
    - hooks:
        - type: command
          command: "SD=\"${CODEX_SKILL_ROOT:-.agents/skills/planning-with-files}/scripts\"; powershell.exe -NoProfile -ExecutionPolicy Bypass -File \"$SD/check-complete.ps1\" 2>/dev/null || sh \"$SD/check-complete.sh\""
metadata:
  version: "2.23.0"
---
...
```powershell
# Windows PowerShell
python ".agents/skills/planning-with-files/scripts/session-catchup.py" (Get-Location)
```
