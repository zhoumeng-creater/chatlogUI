...
describe("diagnostic redaction", () => {
  it("masks data keys, API keys, bearer tokens, and credential-like values", () => {
    ...
    expect(redacted).not.toContain("sk-synthetic-redaction-token");
  ...
  it("masks sensitive local paths and private identity fragments for diagnostics", () => {
    const redacted = maskDiagnosticText(
      "Data dir: C:\\Users\\Synthetic\\Documents\\WeChat Files\\wxid_synthetic_private",
      { privacyMode: true },
    );

    expect(redacted).not.toContain("Alice");
    expect(redacted).not.toContain("WeChat Files");
    expect(redacted).not.toContain("wxid_synthetic_private");
  ...
  it("detects diagnostic text that still contains unsafe private markers", () => {
    ...
    expect(containsSensitiveDiagnosticText("C:\\Users\\Synthetic\\WeChat Files")).toBe(true);
  ...
  it("masks media keys, SNS proxy queries, SQL, raw responses, and request queries", () => {
    const redacted = maskDiagnosticText(
      [
        ...
        "raw response {\"content\":\"synthetic-private-message-should-be-redacted\",\"sender\":\"wxid_synthetic_private\"}",
        "query keyword=synthetic-private-message-should-be-redacted&chat=wxid_synthetic_private",
      ].join("\n"),
      { privacyMode: true },
    );
