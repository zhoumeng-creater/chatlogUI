...
describe("workspaceCommandBarModel", () => {
  ...
  it("keeps disabled reasons explicit instead of pretending unavailable features work", () => {
    const model = buildWorkspaceCommandBar({
    ...
    expect(actions.find((action) => action.id === "export-current")?.disabledReason).toContain("导出任务");
    expect(actions.find((action) => action.id === "jump-date")?.disabledReason).toContain("历史导航");
  });
});
