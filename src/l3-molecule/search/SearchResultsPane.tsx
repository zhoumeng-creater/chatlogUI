...
export interface SearchResultsPaneViewModel {
  ...
  groups: Array<{
    ...
    items: Array<{
      ...
      active: boolean;
      navigation?: SearchResultNavigationState | null;
    }>;
  }>;
}
...
interface SearchResultsPaneProps {
  ...
  onRetryResult?: (message: SearchResults["messages"][number]) => void;
  onOpenNearbyResult?: (message: SearchResults["messages"][number]) => void;
  onLoadMore: () => void;
  onRetry: () => void;
  onClear: () => void;
  onCancelSearch?: () => void;
                ...
                <div key={message.id} role="listitem">
                  <button
                    ...
                    className={classNames(
                      "search-result-row",
                      item.active && "search-result-row--active",
                    )}
                    aria-current={item.active ? "true" : undefined}
                    aria-busy={item.navigation?.status === "loading" ? "true" : undefined}
                    disabled={item.navigation?.status === "loading"}
                  ...
                  {item.navigation?.status === "loading" && (
                    <Typography
                      variant="caption"
                      color="var(--text-secondary)"
                      className="search-result-row__navigation-status"
                    >
                      正在精确定位…
                    </Typography>
                  )}
                  {item.navigation?.status === "error" && (
                    <div className="search-result-row__navigation-error" role="alert">
                      <Typography variant="caption" color="var(--text-secondary)">
                        {item.navigation.message}
                      </Typography>
                      <div className="search-result-row__navigation-actions">
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={() => onRetryResult?.(message)}
                        >
                          重试精确定位
                        </Button>
                        {item.navigation.nearbyFallbackAvailable && onOpenNearbyResult && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onOpenNearbyResult(message)}
                          >
                            按附近时间打开
                          </Button>
                        )}
    ...
    groups: [
      {
        ...
        items: results.messages.map((message) => ({
          ...
          snippetSegments: [
            {
              text: privacyOn
                ? maskDisplayText(message.content || "[空消息]")
                : message.content || "[空消息]",
              highlight: false,
            },
          ],
