...
test("P3-C semantic discovery sends filters, navigates by backend chat, and filters preview", async ({ page }) => {
  ...
  const semanticPanel = page.locator(".semantic-panel");

  await semanticPanel.getByRole("button", { name: "搜索", exact: true }).click();
  await semanticPanel.getByLabel("范围").selectOption("selected");
  await semanticPanel.getByLabel("时间").selectOption("30d");
  await semanticPanel.getByLabel("深度").selectOption("deep");
  await semanticPanel.getByLabel("来源").selectOption("8");
  await semanticPanel.getByLabel("选择搜索会话").getByText("Synthetic Session Alpha").click();
  await semanticPanel.getByLabel("语义搜索查询").fill("synthetic");

  const searchRequest = page.waitForRequest((request) => {
    const url = new URL(request.url());
    return url.pathname.endsWith("/api/v1/semantic/search") && url.searchParams.get("window") === "30d";
  });
  await semanticPanel.locator(".semantic-search__query-row").getByRole("button", { name: "搜索" }).click();
  ...
  expect(searchUrl.searchParams.get("chats")).toBe("session_synthetic_001");
  ...
  await semanticPanel.getByRole("button", { name: "预览", exact: true }).click();
...
test("P3-C semantic discovery shows edge states at narrow width without page overflow", async ({ page }) => {
  ...
  await semanticPanel.getByRole("button", { name: "搜索", exact: true }).click();
  await semanticPanel.getByLabel("语义搜索查询").fill("missing");
  await semanticPanel.locator(".semantic-search__query-row").getByRole("button", { name: "搜索" }).click();
  ...
  await semanticPanel.getByRole("button", { name: "分析", exact: true }).click();
