...
| Media resources | `/image/*key`, `/video/*key`, `/file/*key`, `/voice/*key`, `/data/*path` | Media preview/download with safe failure states | P4-B | Critical | `mediaResources.ts`, `mediaAdapters.ts` | `useMediaCommander`, `useMediaStore` | Endpoint family only, no key/path/query | synthetic metadata only | No CSP change; video/voice playback deferred | verified |
| Chat extensions | `/api/v1/unread`, `/api/v1/members`, `/api/v1/new_messages`, `/api/v1/favorites` | Conversation badges, member inspector, favorites module | P4-B | High | `chatExtensionFetchers.ts`, `chatExtensionsAdapters.ts` | Workbench/chat/favorites commanders and stores | Status, duration, count summary | synthetic rows | None | verified |
...
| Media resources | GET | `/image/*key` | Message attachment preview sheet | media resource fetcher | media commander/store | `advanced-capabilities.json` | loading, error, safe image preview, privacy-on masked preview | verified |
| Media resources | GET | `/video/*key` | Message attachment download placeholder | media resource fetcher | media commander/store | `advanced-capabilities.json` | placeholder/download, load/error, no raw key text; native playback deferred | verified |
| Media resources | GET | `/file/*key` | Message attachment download placeholder | media resource fetcher | media commander/store | `advanced-capabilities.json` | download placeholder, error, no raw key text | verified |
| Media resources | GET | `/voice/*key` | Message attachment download placeholder | media resource fetcher | media commander/store | `advanced-capabilities.json` | placeholder/download, load/error, no raw key text; native playback deferred | verified |
| Media resources | GET | `/data/*path` | Explicit non-rendered backend redirect/data path | media resource fetcher boundary only | media commander/store | `advanced-capabilities.json` | no raw path render or diagnostics; no direct UI route | verified |
| Chat extensions | GET | `/api/v1/unread` | Unread badges/session merge | chat extension fetcher | workbench/chat commander | `advanced-capabilities.json` | count badge, zero state, privacy on/off | verified |
| Chat extensions | GET | `/api/v1/members` | Group member inspector | chat extension fetcher | chat extension commander/store | `advanced-capabilities.json` | empty, list, privacy on/off | verified |
| Chat extensions | GET | `/api/v1/new_messages` | Explicit new message refresh | chat extension fetcher | chat extension commander/store | `advanced-capabilities.json` | incremental rows, dedupe, retry/error | verified |
| Chat extensions | GET | `/api/v1/favorites` | Favorites media/library browser | chat extension fetcher | favorites commander/store | `advanced-capabilities.json` | loading, empty, error, list/detail, privacy on/off | verified |
