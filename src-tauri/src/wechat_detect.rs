...
pub fn detect_wechat_data_dirs() -> Vec<WxPathCandidate> {
    ...
    if let Ok(user_profile) = std::env::var("USERPROFILE") {
        ...
        let onedrive_root = format!("{}\\OneDrive - Default Directory\\Documents\\WeChat Files", user_profile);
        results.extend(scan_wechat_accounts(&onedrive_root));

        let onedrive_xwechat = format!("{}\\OneDrive - Default Directory\\Documents\\xwechat_files", user_profile);
